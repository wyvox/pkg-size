...
function comparePackages(e, t, {
  ...
  sortOrder: s,
  hideFiles: o,
  ignoreThreshold: r = 100
} = {}) {
  const i = {};
  processPkgFiles(i, "head", e), processPkgFiles(i, "base", t);
  const p = Object.values(i);
  sortFiles(p, n, s);
  const [a, u] = partionHidden(o, p), [h, b] = partition$1(u, (y) => y.diff && y.diff.size && Math.abs(y.diff.size.delta) < r);
  return {
    head: e,
    base: t,
...
(async () => {
  ...
  const { pull_request: t } = github.context.payload, n = await generateSizeReport({
    ...
    ignoreThreshold: Number(core.getInput("ignore-threshold") || 100)
  });
